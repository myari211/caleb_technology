...
class Model_menu_frontend extends CI_Model {
	...
	function getListMenu($cond = null){
		 ...
		 $query		= "SELECT menu_id,module_id,menu_title, menu_active_status, menu_parent,menu_sub_parent, menu_url, menu_alias, menu_order,
	...
	function getMenuChildren($parent){
		 $query		= "SELECT a.menu_id,a.module_id, a.menu_title, a.menu_url,a.menu_alias, Deriv1.Count FROM 
	...
	function getMenu($id = '')
	{
		...
		$sql	= "SELECT menu_id, module_id, menu_title, menu_active_status, menu_parent, menu_url,menu_alias, menu_order FROM tbl_menu $where ORDER BY menu_id ASC";	
	...
	function insertMenu($module_id,$menutitle,$menuurl,$menualias,$menuparent,$menusubparent)
	{
		$sql	= "INSERT INTO tbl_menu SET module_id = '".$module_id."', menu_title = '".$menutitle."',
					menu_active_status = 0, menu_url = '".$menuurl."', menu_alias = '".$menualias."', menu_parent = ".$menuparent." ,menu_sub_parent = ".$menusubparent.",
					menu_order = 1,
	...
	function updateMenu($id,$module_id,$menutitle,$menuurl,$menualias,$menuparent,$menusubparent)
	{
		$sql	= "UPDATE tbl_menu SET 
				   module_id='".$module_id."', menu_title='".$menutitle."', menu_url='".$menuurl."',menu_alias='".$menualias."', menu_parent = ".$menuparent.", menu_sub_parent = ".$menusubparent.", 
       ...
       function getParent($menu_id)
	{
        ...
        function GenerateMenu()
            {
		 $data = array();
			 $sql = "SELECT menu_id, menu_title,module_id, menu_active_status, menu_parent,menu_sub_parent, menu_url, menu_alias, menu_order,
					  DATE_FORMAT( menu_create_date, '%d-%m-%Y %H:%i:%s' ) as menu_create_date, menu_create_by
	...
	{
		 $data = array();
			 $sql = "SELECT a.menu_id,a.menu_parent,a.menu_sub_parent,a.menu_title,a.menu_url,a.menu_alias from tbl_menu as a  where   a.menu_sub_parent= 0 and menu_parent= '".$menu_id."'";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
	...
	{
		 $data = array();
			 $sql = "SELECT a.menu_id,a.menu_parent,a.menu_sub_parent,a.menu_title,a.menu_url, a.menu_alias from tbl_menu as a  where  a.menu_sub_parent ='".$menu_id."' and a.menu_parent= '".$menu_parent."'";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
