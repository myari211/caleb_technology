...
function saveTitle(data_id, data_title)
{
	  	...
	  	$.ajax({
			url: '<?php echo BASE_URL_BACKEND;?>/Gallery/saveTitle',
			type: 'post',
...
function saveDescription(data_id, data_description)
{
	  	...
	  	$.ajax({
			url: '<?php echo BASE_URL_BACKEND;?>/Gallery/saveDescription',
			type: 'post',
...
function saveLink(data_id, data_title)
{

	  	$.ajax({
			url: '<?php echo BASE_URL_BACKEND;?>/Gallery/saveLink',
			type: 'post',
			data: {primary_key: data_id, value: data_title},
	...
	<script type='text/javascript'>
		...
		$('.ic-description-field').keyup(function(e) {
    		...
    		$('.ic-description-field').bind({
                ...
                $('.ic-link1-field').keyup(function(e) {
    			if(e.keyCode == 13) {
					var data_id = $(this).attr('data-id');
					var data_title = $(this).val();
                                  //alert(data_title);
					saveLink(data_id, data_title);
    			}
    		});
    		$('.ic-link1-field').bind({
    			  click: function() {
            ...
            <li id="photos_<?php echo $gallery['gallery_id']; ?>">              
				...
				<div class='photo-box'>
					 ...
					 <div class="form-group">
                                        ...
                                        <div class="clear"></div><?php }?>
                                         <?php if($about_order == 3){ ?>
                                         <div class="form-group">
                                              <input type="text" class="ic-link1-field form-control" data-id="<?php echo $gallery['gallery_id']; ?>" value="<?php echo $gallery['gallery_link1']; ?>" autocomplete="off" />
                                        </div>
                                        <div class="clear"></div>
                                         <?php } ?>
