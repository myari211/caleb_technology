...
class Worksgallery extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('backend/Model_menu_frontend','backend/Mupload','backend/Model_gallery_works','backend/Model_alias', 'backend/Model_language','backend/Model_logcms'));
...
//                 $this->section = $gm->module_group_id;
    ...
    public function index($works_id=NULL)
	{
                $_SESSION['works_id'] = $works_id;
		$this->view();
	}
       function view(){
		...
		$cond 			= $where." ".$orderBy;
		$rsUserLevel	= $this->Model_gallery_works->getListGallery($works_id,$cond);
		...
		$ListGallery = $this->Model_gallery_works->getListGallery($works_id, $cond);
		//print_r($ListGallery);
		...
		$this->load->view('backend/works_gallery/list');
	}
	...
	function doupload() {  
            ...
            if ($_FILES['upload_file1']['size'] <= 0) {
                ...
                foreach ($_FILES as $key => $value) {
                    ...
                    if (!empty($value['name'])) {  
                     ...
                     $last_id =  $this->Model_gallery_works->insertGallery($data);
                            ++$i;
                        }
                    }
                }
        ...
        redirect(BASE_URL_BACKEND.'/'.$this->data['controller'].'/'.$works_id);
    }
    public function doOrder(){
		...
		if($order == ""){
			redirect(BASE_URL_BACKEND.'/'.$this->data['controller'].'/'.$_SESSION['works_id']);
			exit();
		} 
		
		foreach($order as $id => $ordervalue){
			$this->Model_gallery_works->updateOrderGallery($id,$ordervalue);
                        echo $ordervalue;
		}
		...
		redirect(BASE_URL_BACKEND.'/'.$this->data['controller'].'/'.$_SESSION['works_id']);
         ...
         function saveTitle(){
            ...
            $this->Model_gallery_works->saveTitle($title,$gallery_id);
           redirect(BASE_URL_BACKEND.'/'.$this->data['controller'].'/'.$_SESSION['works_id']);
    }
     ...
     function saveDescription(){
            ...
            $this->Model_gallery_works->saveDescription($remarks,$gallery_id);
           redirect(BASE_URL_BACKEND.'/'.$this->data['controller'].'/'.$_SESSION['works_id']);
    }
	...
	function active($id){
		...
		$rsContent = $this->Model_gallery_works->getGallery($id); 
		$title = $rsContent[0]['gallery_title'];
		$active_status = abs($rsContent[0]['gallery_active_status']-1);
		
		$active = $this->Model_gallery_works->activeGallery($id);
	...
	function delete($id){
		...
		$rsGallery = $this->Model_gallery_works->getGallery($id);               
		...
		$delete = $this->Model_gallery_works->deleteGallery($id);
