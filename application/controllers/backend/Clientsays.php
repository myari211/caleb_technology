...
class Clientsays extends CI_Controller {
...
//		$cond .= " LIMIT ".$start.",".$per_page;
	...
	public function doAdd(){
		...
		$content_desc = secure_input_editor($_POST['content_desc']);
                $highlight =  secure_input_editor($_POST['content_highlight']);
                if ($highlight == 1){
                    $content_highlight = 1;                    
                    }
                 else {
		...
		if (! count($pesan)==0 ) {
			...
			if ($countContent > 0 ) {
				...
				if ($countAlias > 0 ) {
					...
					$content_id = $this->Model_content->insertContent($menu_id,$page_type,$content_title,$content_desc,$content_highlight,$content_imageurl,$content_alias,$content_metadescription,$content_metakeywords,$content_shortdesc);
					if(!empty($content_id)) {
		...
		if (! count($pesan)==0 ) {
			...
			if ($countContent > 0 ) {
				...
				if ($countAlias > 0 ) {
					...
					$update = $this->Model_content->updateContent($id,$menu_id,$page_type,$content_title,$content_desc,$content_highlight,$content_imageurl,$content_alias,$content_metadescription,$content_metakeywords,$content_shortdesc);
					if($update) {
	...
	public function doOrder(){
        ...
        function generateContent(){
                $ListContent			= $this->Model_content->GenerateContent(" WHERE a.content_active_status = 1 and a.menu_id = 60 ORDER BY a.content_order ASC");
		$countContent		= count($ListContent);
		//createCacheBanner($rsBanner,"bannerhome");
               createCache($ListContent,"client");
        }
}