...
//	public $alias_category = "aboutlsaf";
	...
	public function __construct()
	{
		...
		$this->data['controller'] = $module_name;
                ...
                $this->data['page_type'] = 2;
                $this->data['materialPage'] = 1;
	...
	function view(){
		...
		$ListMaterial = $this->Model_material->getListMaterial($condm);
		$this->data["ListMaterial"] = $ListMaterial;
...
//die;
		...
		$this->data['breadcrump'] = $this->breadcrump;		
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/Content/addmaterial',$this->data);
	}
		...
		$material_title = $this->security->xss_clean(secure_input($_POST['material_title']));
                $highlight =  secure_input_editor($_POST['material_highlight']);
                if ($highlight == 1){
                    $material_highlight = 1;                    
                    }
                 else {
                $material_highlight = 0;
                    }
                $material_imageurl = $this->security->xss_clean(secure_input($_POST['material_imageurl']));
                $insert_file=$this->input->post('file_title');
		$linkData=array();
		$pesan = array();
		// Validasi data
		if ($material_imageurl=="") {
			$pesan[] = 'Image is empty';
		}
		else if ($insert_file=="") {
			$pesan[] = 'File is empty';
		}
		if (! count($pesan)==0 ) {
			foreach ($pesan as $indeks=>$pesan_tampil) {
				$this->data['error'] = $pesan_tampil;
				$this->data['material_title']=$material_title;
				$this->data['material_imageurl']=$material_imageurl;	
				$this->load->view('backend/header',$this->data);
				$this->load->view('backend/Content/addmaterial',$this->data);
			}
		} else {
			$cekMaterial = $this->Model_material->checkMaterial($material_title);
			$countMaterial = count($cekMaterial);
			
			if ($countMaterial > 0 ) {
				$this->data['error']='Material Title '.$material_title.' already exist';
				$this->data['material_title']=$material_title;
				$this->data['material_imageurl']=$material_imageurl;
				$this->load->view('backend/header',$this->data);
				$this->load->view('backend/Content/addmaterial',$this->data);
			} else {
                            $material_imageurl = str_replace(BASE_URL,"",$material_imageurl);
                            $countAlias = 0;	
                            $material_id = $this->Model_material->insertMaterial($material_title,$material_highlight,$material_imageurl);
                                        ...
                                        $today= date("Y-m-d H:i:s");
                                          $data = array();
                                            for ($i = 0; $i < count($this->input->post('file_title')); $i++) {
                                                $checkfile = $this->Model_material_file->checkFile($this->security->xss_clean(secure_input($_POST['file_title'][$i])), $material_id);
                                                if (empty($checkfile)){
                                                   ...
                                                   $data[] = array(
                                                    ...
                                                    'material_file_order'=>1,
                                                    'material_file_active_status'=>0,
...
//                                            die;
					...
					$log_module = "Add ".$this->module;
					$log_value = $material_id." | ".$material_title;
					$insertlog = $this->Model_logcms->insertLogCMS($log_module,$log_value);
		...
		$rsMaterial = $this->Model_material->getMaterial($id);  // mengambil database dari model untuk dikirim ke view
		
                $countMaterial = count($rsMaterial);
		
		$this->data['rsMaterial'] = $rsMaterial;
		$this->data['countMterial'] = $countMaterial;
		
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/Content/editmaterial',$this->data);
	}
       ...
       function doEditMaterial($id){
		...
		$this->data['rsMaterial'] = $rsMaterial;
		$this->data['countMaterial'] = $countMaterial;
		$material_title = $this->security->xss_clean(secure_input($_POST['material_title']));
		$material_titleOld = $this->security->xss_clean(secure_input($_POST['material_titleOld']));		
		$material_imageurl = $this->security->xss_clean(secure_input($_POST['material_imageurl']));
                $highlight =  secure_input_editor($_POST['material_highlight']);
		...
		if (! count($pesan)==0 ) {
			foreach ($pesan as $indeks=>$pesan_tampil) {
				...
				$this->load->view('backend/header',$this->data);
				$this->load->view('backend/Material/edit',$this->data);
			}
		} else {
                    $material_imageurl = str_replace(BASE_URL,"",$material_imageurl);
				$countAlias = 0;
				
				$update = $this->Model_material->updateMaterial($id,$material_title,$material_highlight,$material_imageurl);				
                                $log_module = "Edit ".$this->module;
                                $log_value = $id." | ".$material_title." | ".$material_imageurl;
                                $insertlog = $this->Model_logcms->insertLogCMS($log_module,$log_value);
	...
	function deleteContent($id){
		...
		$rsContent = $this->Model_material->getMaterial($id); 
		$title = $rsContent[0]['material_title'];  
		$delete = $this->Model_material->deleteMaterial($id);
...
//					createCache($ListContent,"content_");
	...
	function doOrderMaterial(){
		...
		foreach($order as $id => $ordervalue){
			$this->Model_material->updateOrderMaterial($id,$ordervalue);
                        echo $ordervalue;
		}
		$this->generateMaterial();
		redirect(BASE_URL_BACKEND.'/'.$this->data['controller']);
         ...
         function generateMaterial(){
                $ListMaterial			= $this->Model_material->GenerateMaterial(" WHERE material_active_status = 1 and material_highlight = 1 and material_image != '' ORDER BY material_order ASC");
		$countMaterial		= count($ListMaterial);
                
               createCache($ListMaterial,"material");
               
                $count			= $this->Model_material->CountMaterial();
                createCache($count,"countmaterial");
        }
				...
				$this->data['quote_desc']=$quote_desc;
				 ...
				 redirect(BASE_URL_BACKEND.'/'.$this->data['controller']);
                  ...
                  //print_r($data);
                    ...
                    $ListHeadQuote= $data;
                    createCache($ListHeadQuote,"material_head_quote");
                    $log_module = "Add ".$this->module;
                    $log_value = "material_head_quote | ".$quote_desc;
                    $insertlog = $this->Model_logcms->insertLogCMS($log_module,$log_value);
