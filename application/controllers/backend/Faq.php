...
//	public $module_id = 15; //get module id from database
	...
	function view(){
		...
		// Validasi data
		if ($title=="") {
			$pesan[] = 'Faq Title is empty';
		} else if ($desc=="") {
			$pesan[] = 'Faq Desc is empty';
		} 
				...
				$this->data['desc']=$desc;
				 ...
				 redirect(BASE_URL_BACKEND.'/'.$this->data['controller']);
                  ...
                  //print_r($data);
                    ...
                    $ListFaqQuote= $data;
                    createCache($ListFaqQuote,"faq");
                    $log_module = "Add ".$this->module;
                    $log_value = "FaqQuote | ".$title." | ".$desc;
