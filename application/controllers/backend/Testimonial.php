...
class Testimonial extends CI_Controller {
	...
	public $alias_category = "testimonial";
	...
	public function doAdd(){
		...
		$tb = $_POST['tbSave'];
		if (!$tb) {
			redirect(BASE_URL_BACKEND."/testimonial");
			exit();
		}
		...
		$this->data['breadcrump'] = $this->breadcrump;
		 ...
		 $testimonial_desc = $this->security->xss_clean(secure_input($_POST['testimonial_desc']));
		
		$pesan = array();
		// Validasi data
		if ($testimonial_desc=="") {
			$pesan[] = 'testimonial is empty';
		} 
		...
		if (! count($pesan)==0 ) {
			foreach ($pesan as $indeks=>$pesan_tampil) {
				...
				$this->data['testimonial_desc']=$testimonial_desc;
				...
				$this->load->view('backend/testimonial/add',$this->data);
                  ...
                  $ListTestimonial= $data;
                    createCache($ListTestimonial,"testimonial");
                    $log_module = "Add ".$this->module;
                    $log_value = "Testimonial | ".$testimonial_title." | ".$testimonial_desc;
