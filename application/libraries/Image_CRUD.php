...
class Image_CRUD {
	...
	protected $description_field = null;
        protected $idcontent_field = null;
	protected $relation_field = null;
	protected $subject = 'Record';
	protected $image_path = '';
	protected $primary_key = 'id_img';
	protected $ci = null;
	...
	function set_description_field($description_field)
	{
        ...
        function set_idcontent_field($idcontent_field)
	{
		$this->idcontent_field = $idcontent_field;
	...
	protected function _library_view($view, $vars = array(), $return = FALSE)
	{
		...
		echo $view_file = 'application/views/';
	...
	public function _insert_description($primary_key, $value)
    {
		$this->ci->db->update($this->table_name, array($this->description_field => $value), array($this->primary_key => $primary_key));
		echo "$this->ci->db->update($this->table_name, array($this->description_field => $value), array($this->primary_key => $primary_key));";
    }
	...
	protected function getState()
	{
		...
		if(isset($rsegments_array[3]) && is_numeric($rsegments_array[3]))
		{
			...
			$insert_description_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/insert_description');
			...
			$state['insert_description_url'] = $insert_description_url;
		...
		{
			$state = array( 'name' => 'delete_file', 'id_img' => $rsegments_array[4]);
			return (object)$state;
		}
			...
			switch ($state_info->name) {
				case 'list':
					...
					$this->_library_view('list.php',array(
						...
						'insert_description_url' => $state_info->insert_description_url,
						'photos' => $photos,
						'ajax_list_url' => $state_info->ajax_list_url,
						'ordering_url' => $state_info->ordering_url,
						'primary_key' => $this->primary_key,
						'title_field' => $this->title_field,
						'description_field' => $this->description_field,
                                                'idcontent_field' => $this->idcontent_field,
				...
				case 'insert_description':
					@ob_end_clean();
					$this->_insert_description($_POST['primary_key'],$_POST['value']);
					die();
				break;
			}
		}
